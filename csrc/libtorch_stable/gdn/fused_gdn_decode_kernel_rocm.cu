 ...
 *
 * ROCm port of the fused GDN decode MTP post-conv kernel
 * (csrc/libtorch_stable/gdn/fused_gdn_decode_kernel.cu).
 *
 * Per (request, value head) block: loads the recurrent state slot selected by
 * the previous step's acceptance, rolls the gated delta-rule recurrence over
 * the request's verify tokens, snapshots intermediate states into the draft
 * slot rows, and applies the gated RMSNorm epilogue -- replacing the Triton
 * decode chain (rearrange glue + fused_sigmoid_gating_delta_rule_update +
 * layernorm) in every GDN layer of the spec-decode step.
 *
 * gfx908 has no cp.async; state chunks are staged through a double-buffered
 * shared array with synchronous 16-byte copies. As in the CUDA original, the
 * copy for chunk c+1 targets the opposite buffer from the one chunk c reads,
 * so the staging overlap needs only one barrier per chunk.
 */
...
// ROCm's warp-sync builtins require a 64-bit lane mask.
#ifdef USE_ROCM
#define GDN_WARP_MASK uint64_t(-1)
#else
#define GDN_WARP_MASK 0xffffffffu
#endif
...
__device__ __forceinline__ float load_dt_bias(const void* dt_bias, int head,
  ...
  if (dt_bias_type == kDtBiasBFloat16) {
    return __bfloat162float(static_cast<const bf16_t*>(dt_bias)[head]);
  }
...
#pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
    value += __shfl_xor_sync(GDN_WARP_MASK, value, offset);
  }
  return value;
}
...
#pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
    x += __shfl_xor_sync(GDN_WARP_MASK, x, offset);
    y += __shfl_xor_sync(GDN_WARP_MASK, y, offset);
  }
  return {x, y};
}
...
__device__ __forceinline__ float4 load_state4<bf16_t>(const bf16_t* state) {
  const __hip_bfloat162 lo = *reinterpret_cast<const __hip_bfloat162*>(state);
  const __hip_bfloat162 hi = *reinterpret_cast<const __hip_bfloat162*>(state + 2);
...
__device__ __forceinline__ void store_state4<bf16_t>(bf16_t* state,
                                                     float4 value) {
  *reinterpret_cast<__hip_bfloat162*>(state) =
      __float22bfloat162_rn(make_float2(value.x, value.y));
  *reinterpret_cast<__hip_bfloat162*>(state + 2) =
      __float22bfloat162_rn(make_float2(value.z, value.w));
}

// Synchronous 16-byte global -> shared staging of one state chunk.
template <typename StateT, int ChunkV, int DimK>
__device__ __forceinline__ void copy_state_chunk(StateT* shared_state,
  ...
  uint4* dst = reinterpret_cast<uint4*>(shared_state);
  const uint4* src = reinterpret_cast<const uint4*>(
      state + static_cast<int64_t>(chunk) * ChunkV * DimK);
...
__global__ __launch_bounds__(kThreads) void gdn_decode_post_conv_mtp_kernel(
    const bf16_t* __restrict__ mixed_qkv, const bf16_t* __restrict__ a,
    const bf16_t* __restrict__ b, const float* __restrict__ a_log,
    ...
    const bf16_t* __restrict__ output_gate, const void* __restrict__ norm_weight,
    bf16_t* __restrict__ out, int H, int HV, int state_indices_width,
  ...
  copy_state_chunk<StateT, kChunkV, kDimK>(&shared_state[0][0][0], source_state,
                                           0, tid, kThreads);
...
#pragma unroll
    ...
    // The butterfly reduce leaves the full sums in every lane; do not
    // broadcast via __shfl_sync(srcLane=0) -- on wave64 parts (gfx908) the
    // source lane is wave-relative and would read another 32-thread warp's
    // scale.
    const float q_scale = rsqrtf(qk_sums.x + 1.0e-6f) * scale;
    const float k_scale = rsqrtf(qk_sums.y + 1.0e-6f);
#pragma unroll
    for (int i = 0; i < 4; ++i) {
      const int dim = lane + i * 32;
...
#pragma unroll 1
  for (int chunk = 0; chunk < kNumChunks; ++chunk) {
    if (chunk > 0) {
      copy_state_chunk<StateT, kChunkV, kDimK>(&shared_state[chunk & 1][0][0],
                                               source_state, chunk, tid,
                                               kThreads);
      __syncthreads();
    }
...
#pragma unroll
    for (int i = 0; i < 4; ++i) {
      ...
      const float weight =
          norm_weight_is_bf16
              ? __bfloat162float(static_cast<const bf16_t*>(norm_weight)[value])
...
void launch_gdn_decode_post_conv_mtp(
  ...
  cudaStream_t stream = get_current_cuda_stream(mixed_qkv.get_device_index());
...
void fused_gdn_decode_post_conv_mtp(
  ...
  const auto dispatch_state_type = [&]<int ValueHeadsPerKeyHead>() {
    if (state_scalar_type == ScalarType::Float) {
      ...
      launch.template operator()<bf16_t, ValueHeadsPerKeyHead>();
    }
  };
