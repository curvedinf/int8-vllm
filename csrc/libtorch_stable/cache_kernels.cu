...
void swap_blocks_batch(const torch::stable::Tensor& src_ptrs,
  ...
  if (batch_fn != nullptr && usable_stream) {
    ...
    // The driver reads one attrIdxs entry per descriptor; a single scalar
    // is an out-of-bounds read for cnt > 1 (vLLM #53863 / fix PR #53971).
  ...
  // rocm-7.14+ has better performance.
  {
    ...
    // One attrIdxs entry per descriptor (ignored when num_attrs == 0);
    // a single scalar is an out-of-bounds driver read for cnt > 1
    // (upstream vLLM #53863 / fix PR #53971).
    std::vector<size_t> attr_idxs;
    size_t fail_idx = 0;
    size_t num_attrs = 0;
