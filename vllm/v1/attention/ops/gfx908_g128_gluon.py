...
"""gfx908 split-K decode core for two-group int8 KV and 256-wide heads."""
...
def g128_core(
    ...
    acc1 = gl.full((32, 128), 0.0, gl.float32, layout=mma)
    ...
    for j in range(lo, hi):
        ...
        score0 = gl.full((32, 32), 0.0, gl.float32, layout=mma)
        score1 = gl.full((32, 32), 0.0, gl.float32, layout=mma)
        ...
        mask = (
            score_qvalid[:, None]
            & (kvpos[None, :] < max_prefix)
            & (kvpos[None, :] <= context + score_qpos[:, None])
        )
