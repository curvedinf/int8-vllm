...
start_server() {
  ...
  printf 'starting direwolf Qwen3.6 server: url=http://%s:%s cpuset=%s log=%s/server.log\n' \
...
status_server() {
  if is_running; then
    ...
    rg -n 'Model loading took|Available KV cache memory|GPU KV cache size|Maximum concurrency for 65,536|via P2P/IPC|Uvicorn running' \
