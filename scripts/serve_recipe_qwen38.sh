...
start_server() {
  ...
  VLLM_STEP_STATE_DUMP="${VLLM_STEP_STATE_DUMP:-}" \
  VLLM_SLOT_DEBUG="${VLLM_SLOT_DEBUG:-}" \
  VLLM_SLSSTAT="${VLLM_SLSSTAT:-}" \
  VLLM_OFFLOAD_LOADLOG="${VLLM_OFFLOAD_LOADLOG:-0}" \
  ...
  VLLM_STEPTRACE="${VLLM_STEPTRACE:-}" \
  VLLM_ATTNTRACE="${VLLM_ATTNTRACE:-}" \
  VLLM_ATTN_PACE="${VLLM_ATTN_PACE:-}" \
  VLLM_STEPPHASE="${VLLM_STEPPHASE:-}" \
  VLLM_CGDISPATCH_TRACE="${VLLM_CGDISPATCH_TRACE:-}" \
  VLLM_STEPBREAK="${VLLM_STEPBREAK:-}" \
  VLLM_SCHED_NO_MIX="${VLLM_SCHED_NO_MIX:-}" \
  VLLM_GDN_DECODE_KERNEL="${VLLM_GDN_DECODE_KERNEL:-triton}" \
  VLLM_GDNSTAT_SLICE_ALL="${VLLM_GDNSTAT_SLICE_ALL:-}" \
  ...
  VLLM_WEIGHTHASH="${VLLM_WEIGHTHASH:-}" \
  VLLM_ACCEPTLOG="${VLLM_ACCEPTLOG:-}" \
  VLLM_GDN_PREFILL_EXACT="${VLLM_GDN_PREFILL_EXACT:-0}" \
  VLLM_GDN_PREFILL_EXACT_MIN_T="${VLLM_GDN_PREFILL_EXACT_MIN_T:-}" \
  VLLM_GDN_PREFILL_FP32="${VLLM_GDN_PREFILL_FP32:-1}" \
