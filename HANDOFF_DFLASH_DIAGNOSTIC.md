...
## TG performance investigation (2026-08-27, corrected record)

CORRECTION of the first-pass attribution: the 105W power cap is NOT the
problem and is production policy (never touched). The evidence table:

| bench | time (Aug 26) | TPOT | C8 TG | vllm/aiter SHA |
|---|---|---|---|---|
| stock-power-...T200301Z | 20:03:04 | 8.23 ms | 492 tok/s | 9682fd5e/1353f63c |
| lowpower-pp2k-...T200422Z | 20:04:22 | 42.87 ms | 75.6 tok/s | same |
| lowpower-pp2k-r2 | 20:14 | 42.79 ms | 58.7 tok/s | same |

Same code 78s apart: the 20:03 sysfs write forced the clock GOVERNOR to
manual with sclk pinned at level 0. Under that pin the GPUs cannot ramp
under load (measured 18 TOPS / 33-66W at 100% util sustained; a ramping
part sits at its power cap). Every sub-15ms bench predates the write;
every bench after it measured 24-43ms (incl. gapcheck 38.04ms on
reverted code and csvfix 24.95ms with tuned rows).

Two landed fixes:
1. aiter 7c3ab7321 — tuned splitK rows for all serving (N,K) x padded-M
   grid (was: every serving GEMM on splitK=0 defaults since the Aug 26
   rebuild; miss-log flood). Kernel-level timings strictly better per
   shape (splitK sweep, deterministic); C8-level effect is within this
   box's boot variance (88-217 tok/s across boots either way) — do NOT
   cite as an end-to-end win.
2. serve script clock_state_guard — when started as ROOT (production
   workflow) restores ONLY power_dpm_force_performance_level=auto so
   clocks can ramp WITHIN the existing cap; power1_cap is read before
   and after and asserted unchanged. Non-root start prints a NOTICE
   with the one-liner.

Bottom line: the pinned-governator state explains the entire gap
(stock 8.23ms x ~5 = 41ms = measured 38-43ms across all post-20:03
boots, rows or no rows). Validation at the CURRENT governor state:
TPOT 38-42ms / 88-104 tok/s C8, single-stream 36-37 tok/s, long-ctx
(~7k) 19-23 tok/s. Restoration happens on the next root start of the
serve script (guard), after which stock-power-class numbers are
expected (492 tok/s was measured at cap 290W; at the 105W production
cap expect lower but far above today's).
